...
class GoodsCategory(BaseModel):          # 商品分类表对应的内容, 自关联
    name = models.CharField(max_length=10, verbose_name='名称')# 分类的名称
    # 分类的上级id (分类一共有三级)
...
class GoodsChannel(BaseModel):                                                   #商品频道表展示的内容
    group_id = models.IntegerField(verbose_name='组号')# 当前商品频道属于哪个组
    # 频道对应的一级分类id
    category = models.ForeignKey(GoodsCategory, on_delete=models.CASCADE,  verbose_name='顶级商品类别')
    url = models.CharField(max_length=50,  verbose_name='频道页面链接') # 当前频道点击后跳转的链接地址
    sequence = models.IntegerField(verbose_name='组内顺序')# 这组频道的先后顺序
    class Meta:
...
class Brand(BaseModel):                                     # 品牌
    name = models.CharField(max_length=20,  verbose_name='名称')# 品牌名称
    logo = models.ImageField(verbose_name='Logo图片') # 品牌logo
    first_letter = models.CharField(max_length=1,  verbose_name='品牌首字母')  # 品牌首字母
    class Meta:
        db_table = 'tb_brand'
        verbose_name = '品牌'
        verbose_name_plural = verbose_name
...
class Goods(BaseModel):               #商品SPU表对应的内容: SPU: 代表一组商品
    name = models.CharField(max_length=50,  verbose_name='名称')# 这组商品的名称
    brand = models.ForeignKey(Brand,   on_delete=models.PROTECT,  verbose_name='品牌')# 这组商品的品牌
    category1 = models.ForeignKey(GoodsCategory, on_delete=models.PROTECT, related_name='cat1_goods', verbose_name='一级类别')# 这组商品的一级分类
    category2 = models.ForeignKey(GoodsCategory,  on_delete=models.PROTECT, related_name='cat2_goods', verbose_name='二级类别') # 这组商品的二级分类
...
class SKU(BaseModel):        #商品SKU表对应的内容:   SKU: 具体的某个商品
    ...
    market_price = models.DecimalField(max_digits=10,  decimal_places=2,   verbose_name='市场价')# 此商品的市场价
    stock = models.IntegerField(default=0, verbose_name='库存') # 此商品的库存
    sales = models.IntegerField(default=0, verbose_name='销量') # 此商品的销量
    comments = models.IntegerField(default=0, verbose_name='评价数')# 此商品的评价个数
    is_launched = models.BooleanField(default=True, verbose_name='是否上架销售')# 此商品是否上架(是否在售)
    default_image_url = models.CharField(max_length=200,default='', null=True,blank=True,verbose_name='默认图片')# 此商品对应的图片
    class Meta:
        db_table = 'tb_sku'
        verbose_name = '商品SKU'
        verbose_name_plural = verbose_name
...
class SKUImage(BaseModel):            #SKU图片
    sku = models.ForeignKey(SKU,  on_delete=models.CASCADE, verbose_name='sku')# 这个图片对应哪个 SKU 商品类
    image = models.ImageField(verbose_name='图片')# 图片
    class Meta:
